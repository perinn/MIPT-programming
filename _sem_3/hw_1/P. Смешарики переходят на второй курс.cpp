...
SegmentTree::SegmentTree(const std::vector<int64_t>& arr) {
  ...
  tree_ = std::vector<Node>(2 * n_ - 1, {neutral_, neutral_, 1});
  ...
  for (int64_t i = n_ - 2; i >= 0; --i) {
    ...
    tree_[i].cnt = tree_[Left(i)].cnt + tree_[Right(i)].cnt;
  }
}

void SegmentTree::Push(int64_t node) {
  tree_[node].value = Plus(tree_[node].value, tree_[node].cnt * tree_[node].promise);
...
int64_t SegmentTree::Query(int64_t l, int64_t r, int64_t node, int64_t a, int64_t b) {
  ...
  if (a >= l && b <= r) {
    return Plus(tree_[node].value, tree_[node].cnt * tree_[node].promise);
  }
  if (l > b || a > r) {
    return neutral_;
  }
...
void SegmentTree::Update(int64_t l, int64_t r, int64_t node, int64_t a, int64_t b, int64_t delta) {
  ...
  tree_[node].value = Max(Query(a, static_cast<int64_t>(std::floor((a + b) / 2)), Left(node), a,
                                static_cast<int64_t>(std::floor((a + b) / 2))),
                          Query(static_cast<int64_t>(std::floor((a + b) / 2)) + 1, b, Right(node),
                                static_cast<int64_t>(std::floor((a + b) / 2)) + 1, b));
}
...
int main() {
  ...
  for (int64_t i = 0; i < q; ++i) {
    std::cin >> com >> l >> r;
    if (com == 2) {
      std::cout << segment_tree.Query(l, r - 1, 0, 0, -1) << '\n';
    } else if (com == 1) {
      std::cin >> add;
      segment_tree.Update(l, r - 1, 0, 0, -1, add);
    }
  }
  return 0;
}